...
SHEET_CSV_URL = os.environ.get(
    ...
    'https://docs.google.com/spreadsheets/d/e/2PACX-1vRQDMPDHu9vhPVw_XWFGH4xoOHcV5ugsGh1V3JaQc70_zkSO9PU1qtg7qPbFjnpuaOZOUh_GPkpR6nb/pub?output=csv',
).strip()
...
def load_entries(path: Path) -> list[dict]:
    ...
    approved_column = next((name for name in fieldnames if clean(name).lower() == 'approved'), None)
    approval_enabled = approved_column is not None

    entries = []
    for row in rows:
        if approval_enabled:
            approved = clean(row.get(approved_column, '')).lower()
            if approved not in {'yes', 'y', 'true', '1', 'approved'}:
                continue
...
def render(entries: list[dict]) -> str:
    ...
    category_buttons = ''.join(
        f'<button class="filter-pill" data-category="{esc(cat)}"><span style="background:{CATEGORY_COLORS.get(cat, "#64748B")}"></span>{esc(cat)} <b>{count}</b></button>'
    ...
    city_list = ', '.join(f'{esc(city)} ({count})' for city, count in cities if city)
    ...
    for e in entries:
        ...
        photo = '<span class="mini-badge">Photo on file</span>' if e['has_photo'] else ''
...
<article class="card" data-category="{esc(cat)}" data-search="{esc(e['search_blob'])}">
  ...
  <div class="meta-row"><span>{esc(e['city'])}</span><span>{esc(cat)}</span>{photo}</div>
...
.controls {{ margin-top:18px; background:rgba(255,255,255,.92); border:1px solid rgba(238,221,247,.95); border-radius:24px; padding:18px; box-shadow:0 18px 46px rgba(91,33,182,.08); position:sticky; top:12px; z-index:10; backdrop-filter:blur(10px); }}
.search-row {{ display:grid; grid-template-columns:1fr auto; gap:12px; }}
.search-row input {{ width:100%; border:1px solid var(--line); border-radius:999px; padding:13px 18px; font-size:15px; outline:none; }}
...
.filter-pill {{ border:1px solid var(--line); background:white; border-radius:999px; padding:8px 11px; display:inline-flex; align-items:center; gap:7px; color:#334155; cursor:pointer; }}
.filter-pill span {{ width:9px; height:9px; border-radius:50%; display:inline-block; }} .filter-pill.active {{ background:#1f2350; color:white; border-color:#1f2350; }}
...
.meta-row {{ display:flex; flex-wrap:wrap; gap:7px; margin-top:14px; }} .meta-row span,.mini-badge {{ display:inline-flex; align-items:center; border:1px solid #E9EAF3; background:#FAFAFF; color:#475569; border-radius:999px; padding:5px 9px; font-size:12px; font-weight:700; }}
...
.footer {{ padding:0 24px 32px; color:#64748B; font-size:12px; line-height:1.55; }} .no-results {{ display:none; background:white; border:1px solid var(--line); border-radius:24px; padding:28px; text-align:center; color:var(--muted); }} .no-results.show {{ display:block; }}
...
<body>
<header class="hero"><div class="hero-inner"><div class="eyebrow">Updated monthly · {esc(edition)}</div>{cover}<div class="hero-copy"><div><h1>{esc(TITLE)}</h1><p>{esc(DESCRIPTION)}</p><div class="tagline">Find community. Share resources. We’re stronger together.</div></div><div class="stats"><div class="stat"><strong>{len(entries)}</strong><span>Listings</span></div><div class="stat"><strong>{len(categories)}</strong><span>Categories</span></div><div class="stat"><strong>{len([c for c,_ in cities if c])}</strong><span>Cities / Areas</span></div><div class="stat"><strong>{esc(updated)}</strong><span>Last update</span></div></div></div></div></header>
<main class="shell"><section class="controls"><div class="search-row"><input id="search" type="search" placeholder="Search by name, business, service, category, or city..."><button id="clear" class="clear-btn">Clear</button></div><div class="filters"><button class="filter-pill active" data-category="all"><span style="background:#1f2350"></span>All <b>{len(entries)}</b></button>{category_buttons}</div></section><section class="layout"><aside class="sidebar"><h2>Category index</h2><p>Use filters online, or scan this index in print.</p>{category_summary}<div class="city-note"><strong>Areas represented:</strong><br>{city_list}</div></aside><section><div id="cards" class="cards">{''.join(cards)}</div><div id="noResults" class="no-results">No listings match your current search.</div></section></section></main>
<footer class="footer"><p><strong>Monthly update note:</strong> This directory is generated from the linked Google Form responses sheet. The form login/email field is not displayed.</p></footer>
<script>
window.directoryEntries = {entries_json};
const search = document.getElementById('search'), clear = document.getElementById('clear'), pills = [...document.querySelectorAll('.filter-pill')], cards = [...document.querySelectorAll('.card')], breaks = [...document.querySelectorAll('.category-break')], noResults = document.getElementById('noResults');
let activeCategory = 'all';
function applyFilters() {{ const q = search.value.trim().toLowerCase(); let visible = 0; cards.forEach(card => {{ const show = (activeCategory === 'all' || card.dataset.category === activeCategory) && (!q || card.dataset.search.includes(q)); card.hidden = !show; if (show) visible++; }}); breaks.forEach(block => {{ const cat = block.dataset.categoryBlock; block.hidden = !cards.some(card => !card.hidden && card.dataset.category === cat); }}); noResults.classList.toggle('show', visible === 0); }}
pills.forEach(pill => pill.addEventListener('click', () => {{ pills.forEach(p => p.classList.remove('active')); pill.classList.add('active'); activeCategory = pill.dataset.category; applyFilters(); }}));
search.addEventListener('input', applyFilters); clear.addEventListener('click', () => {{ search.value = ''; activeCategory = 'all'; pills.forEach(p => p.classList.toggle('active', p.dataset.category === 'all')); applyFilters(); }});
</script>
</body>
</html>'''
